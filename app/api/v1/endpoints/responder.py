...
async def send_otp(otp_request:ResponderOTPRequest, db: AsyncSession = Depends(get_db)) -> ResponderOTPResponse:
    ...
    return ResponderOTPResponse(success=is_success, message=message)
...
async def verify_otp(verify_request: ResponderOTPVerifyRequest, db: AsyncSession = Depends(get_db)) -> ResponderOTPVerifyResponse:
    ...
    return ResponderOTPVerifyResponse(success=is_success, message=message, send_again=send_again)
